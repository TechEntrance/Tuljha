...
function Organizations() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!user?.id) {
      toast.error('You must be logged in to add organizations');
      return;
    }
  ...
  const handleDelete = async (id: number) => {
    try {
      ...
      const hasOrders = await db.foodOrders.where('organizationId').equals(id).count() > 0;
      if (hasOrders) {
        toast.error('Cannot delete organization with existing orders');
        return;
      }
